...
const routeSymbolHas = (_target: unknown, prop: string | symbol): boolean =>
  prop === EDEN_ROUTE_SYMBOL;

export function getRouteMeta(enhanced: unknown): RouteMeta | undefined {
  if (
    enhanced !== null &&
    (typeof enhanced === "object" || typeof enhanced === "function") &&
    EDEN_ROUTE_SYMBOL in (enhanced as object)
  ) {
...
function createProxyInternal(client: unknown, segments: readonly string[]): unknown {
  return new Proxy(function () {} as never, {
    ...
    apply(_target, _thisArg, args: unknown[]) {
      ...
      const newSegments =
        serializedSegment !== undefined ? [...segments, serializedSegment] : segments;
