<template>
  <div>
    <div class="page-header">
      ...
      <p class="page-desc">
        SSR-hydrated queries, zero-config cache invalidation, and end-to-end type safety powered by
        <strong style="color: rgba(255, 255, 255, 0.7)">Eden Treaty</strong> +
        <strong style="color: rgba(255, 255, 255, 0.7)">TanStack Query v5</strong>.
      </p>
    </div>
    ...
    <div class="grid-3 section">
      ...
      <div class="stat-card">
        ...
        <div
          class="stat-value"
          style="display: flex; align-items: center; gap: 10px; font-size: 1.1rem; margin-top: 4px"
        >
    ...
    >
      ...
      <div>
        <div style="font-size: 0.8rem; font-weight: 600; color: #f1f5f9">
          SSR hydration — zero flicker
        </div>
        <div style="font-size: 0.72rem; color: rgba(255, 255, 255, 0.35); margin-top: 2px">
          The Nuxt plugin dehydrates the query cache on the server and hydrates it on the client.
          Data is never double-fetched.
          <code
            style="
              color: rgba(52, 211, 153, 0.65);
              font-family: &quot;JetBrains Mono&quot;, monospace;
            "
            >staleTime: 5000ms</code
          >
          prevents immediate refetch after hydration.
        </div>
      </div>
    ...
    <div class="grid-3 section">
      ...
      <div class="feature-card">
        ...
        <div class="feature-desc">
          Type inference flows from the Elysia server definition through Eden to the Vue Query
          composables. Zero
          <code
            style="
              font-family: &quot;JetBrains Mono&quot;, monospace;
    ...
    <div class="card section">
      ...
      <div class="code-block">
        <span class="cm">// composables/eden.ts</span> <span class="kw">import</span> { treaty }
        <span class="kw">from</span> <span class="str">'@elysiajs/eden'</span>
        <span class="kw">import</span> { createEdenQueryHelpers } <span class="kw">from</span>
        <span class="str">'@elysia-vue-query/vue'</span> <span class="kw">import type</span> { App }
        <span class="kw">from</span> <span class="str">'@playground/api'</span>

        <span class="kw">const</span> client = treaty<span class="op">&lt;</span>App<span class="op"
          >&gt;</span
        >(<span class="str">'http://localhost:3000'</span>)
        <span class="kw">export const</span> eden =
        <span class="fn">createEdenQueryHelpers</span>(client)

        <span class="cm">// In your component — fully typed, auto-invalidating</span>
        <span class="kw">const</span> { <span class="prop">data</span>: users } = eden.<span
          class="fn"
          >useQuery</span
        >(eden.proxy.users.get) <span class="kw">const</span> mutation = eden.<span class="fn"
          >useMutation</span
        >(eden.proxy.users.post)

        <span class="cm">// Create → users.get auto-refetches ✨</span>
        mutation.<span class="fn">mutate</span>({ <span class="prop">name</span>:
        <span class="str">'Alice'</span>, <span class="prop">email</span>:
    ...
    <div class="grid-2 section">
      ...
      <div class="card">
        ...
        <div v-if="postsStatus === 'pending'" class="loading">
          <div
            class="spinner"
            style="border-top-color: #a78bfa; border-color: rgba(167, 139, 250, 0.18)"
          />
          Loading…
        </div>
