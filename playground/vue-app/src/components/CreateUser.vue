  ...
  <form @submit.prevent="submit" style="margin-bottom:1rem;">
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.5rem;margin-bottom:0.5rem;">
      <div>
        ...
        <input v-model="name" class="form-input" type="text" placeholder="Jane Doe" required :disabled="mutation.isPending.value" />
      </div>
      <div>
        <label class="form-label">Email</label>
        <input v-model="email" class="form-input" type="email" placeholder="jane@example.com" required :disabled="mutation.isPending.value" />
      </div>
    </div>
    <div style="display:flex;align-items:center;gap:0.75rem;">
      ...
      <Transition name="pop">
        <span v-if="showSuccess" style="font-size:0.75rem;color:#34d399;display:flex;align-items:center;gap:4px;">
