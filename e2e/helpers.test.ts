...
function createRealisticClient() {
  ...
  const posts = [
    { id: 1, userId: 1, title: 'Getting Started with Elysia', body: 'Elysia is a fast, type-safe web framework...' },
    { id: 2, userId: 1, title: 'Eden Treaty Deep Dive', body: 'Eden Treaty provides end-to-end type safety...' },
    { id: 3, userId: 2, title: 'Vue Query Patterns', body: 'TanStack Query for Vue brings powerful data fetching...' },
  ]
...
describe('helpers integration: proxy + key + endpoint call', () => {
  it('proxy endpoint is callable and returns Eden-shaped response', async () => {
    ...
    const response = await (proxy.users.get as unknown as () => Promise<{ data: unknown; error: unknown }>)()
  ...
  it('proxy parameterized endpoint retrieves specific data', async () => {
    ...
    const response = await (endpoint as unknown as () => Promise<{ data: unknown; error: unknown }>)()
    expect(response.error).toBeNull()
  ...
  it('proxy mutation endpoint accepts body and modifies state', async () => {
    ...
    const response = await (endpoint as unknown as (body: unknown) => Promise<{ data: unknown; error: unknown }>)({
      name: 'Dave',
    ...
    const listResponse = await (proxy.users.get as unknown as () => Promise<{ data: unknown[]; error: unknown }>)()
  ...
  it('full mutation workflow: key + mutate + invalidation key', async () => {
    ...
    // Call mutation
    const response = await (mutationEndpoint as unknown as (body: unknown) => Promise<{ data: unknown; error: unknown }>)({
      name: 'Eve',
  ...
  it('error response is correctly surfaced', async () => {
    ...
    const response = await (endpoint as unknown as () => Promise<{ data: unknown; error: unknown; status: number }>)()
  ...
  it('delete mutation modifies state and returns deleted entity', async () => {
    ...
    // Get initial count
    const beforeResponse = await (proxy.users.get as unknown as () => Promise<{ data: unknown[] }>)()
    const initialCount = beforeResponse.data.length

    // Delete user 1
    const deleteEndpoint = proxy.users({ id: '1' }).delete
    const response = await (deleteEndpoint as unknown as () => Promise<{ data: unknown; error: unknown }>)()
    expect(response.error).toBeNull()
    expect(response.data).toMatchObject({ id: 1 })

    // Verify state changed
    const afterResponse = await (proxy.users.get as unknown as () => Promise<{ data: unknown[] }>)()
